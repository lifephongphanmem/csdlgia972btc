...
class CbKkVtXTxController extends Controller
{
    public function index(){
        $model = Company::where('level','DVVT')
            ->where('vtxtx','1')
            ->get();
        return view('congbo.dvvt.vtxtx.index')
            ->with('model',$model)
            ->with('pageTitle','Thông tin doanh nghiệp vận tải xe taxi');
    }
    ...
    public function show($maxa){
        ...
        $modelcb = CbKkDvVtXtx::where('maxa',$maxa)
            ->first();
        if(count($modelcb))
            $modelcbct = KkDvVtXtxCt::where('masokk',$modelcb->masokk)
                ->get();
        else
            $modelcbct = '';
        return view('congbo.dvvt.vtxtx.show')
    ...
    public function history($maxa){
        $model = KkDvVtXtx::where('trangthai','Đã duyệt')
            ->where('maxa',$maxa)
            ->get();
        $modeldn = Company::where('maxa',$maxa)->first();
        return view('congbo.dvvt.vtxtx.history')
            ->with('model',$model)
            ->with('modeldn',$modeldn)
    ...
    public function showttkk(Request $request){
        ...
        if(isset($inputs['id'])){
            $modelkk = KkDvVtXtx::where('id',$inputs['id'])
                ->first();
            $model = KkDvVtXtxCt::where('mahs',$modelkk->mahs)
                ->get();
