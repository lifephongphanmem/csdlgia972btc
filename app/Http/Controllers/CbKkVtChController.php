...
class CbKkVtChController extends Controller
{
    public function index(){
        $model = Company::where('level','DVVT')
            ->where('vtch','1')
            ->get();
        return view('congbo.dvvt.vtch.index')
            ->with('model',$model)
            ->with('pageTitle','Thông tin doanh nghiệp vận tải khác (chở hàng)');
    }
    ...
    public function show($maxa){
        ...
        $modelcb = CbKkDvVtKhac::where('maxa',$maxa)
            ->first();
        if(count($modelcb))
            $modelcbct = KkDvVtKhacCt::where('masokk',$modelcb->masokk)
                ->get();
        else
            $modelcbct = '';
        return view('congbo.dvvt.vtch.show')
